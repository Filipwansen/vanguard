...
Route::group(['middleware' => ['auth', 'verified']], function () {
    ...
    * ocr
    */
    Route::get('ocrupload', 'OcrLabelingController@ocr_upload');
    ...
    * OCR Labeling
    */
    Route::get('ocr', 'OcrLabelingController@index')->middleware('check.ocr.key');
    Route::get('ocr/invalid', 'OcrLabelingController@invalid_ocr');        
    Route::get('template', 'OcrLabelingController@template_index')->name('index.template');
    Route::get('template/{id}/{slug}', 'OcrLabelingController@perform')->name('slug.template');
    Route::post('update/template/{id}', 'OcrLabelingController@update_post_template')->name('update.template'); 
    Route::delete('template/{id}', 'OcrLabelingController@delete_template');
    Route::get('download/template/{id}', 'OcrLabelingController@download')->middleware('check.ocr.key');

    /**
    * company Manage
    */
    Route::group(['middleware'=>'permission:company.manage'], function(){
        Route::get('company', 'CompaniesController@index');
        Route::get('company/{id}/{slug}', 'CompaniesController@perform')->name('slug.company');
        Route::delete('company/{id}', 'CompaniesController@delete_company');
        Route::get('add/company', 'CompaniesController@add_get_company')->name('add.company');
        Route::post('add/company', 'CompaniesController@add_post_company')->name('post.company');
        Route::post('update/company/{id}', 'CompaniesController@update_post_company')->name('update.company'); 
        Route::get('keygen', 'CompaniesController@keygen')->name('ocr.key.company');       
        Route::post('keygen', 'CompaniesController@save_keygen')->name('post.key.company');       
    });

    /**
    * API - how to
    */
    Route::get('api', 'OcrLabelingController@api_howto')->name('api.index');
    Route::get('api/test', 'OcrLabelingController@api_test')->name('api.test');   

});
//test
Route::get('folder/{companyName}', 'CompaniesController@CreateCompanyFolder');
Route::get('rfolder/{companyName}', 'CompaniesController@RemoveCompnayFolder');
